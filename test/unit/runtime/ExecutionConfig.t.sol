...
/// @title ExecutionConfigTest
/// @notice Unit tests for ExecutionConfig contract
contract ExecutionConfigTest is Test {
    ExecutionConfig public config;
    ...
    function test_RevertWhen_Initialize_AlreadyInitialized() public {
        ...
        vm.prank(SystemAddresses.GENESIS);
        vm.expectRevert(Errors.ExecutionConfigAlreadyInitialized.selector);
    ...
    function test_Event_Initialize() public {
        ...
        emit ExecutionConfig.ExecutionConfigUpdated(keccak256(INITIAL_CONFIG));
        config.initialize(INITIAL_CONFIG);
    }
    ...
    function test_Event_SetForNextEpoch() public {
        ...
        vm.prank(SystemAddresses.GOVERNANCE);
        vm.expectEmit(true, false, false, false);
        emit ExecutionConfig.PendingExecutionConfigSet(keccak256(NEW_CONFIG));
        config.setForNextEpoch(NEW_CONFIG);
    }
    ...
    function test_Event_PendingExecutionConfigCleared() public {
        _initializeConfig();
