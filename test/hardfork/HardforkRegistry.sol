...
library HardforkRegistry {
    ...
    // ========================================================================
    // EPSILON HARDFORK DEFINITION
    // ========================================================================

    /// @notice Build the Epsilon hardfork definition
    /// @dev Epsilon upgrades from gravity-testnet-v1.3 to v1.4 (PR #56, #63, #66):
    ///      - ValidatorManagement: D3-2 underbonded eviction (Phase 1) + percentage threshold
    ///        (Phase 2), skip epoch 1, new ValidatorUnderbondedEvicted event
    ///      - Reconfiguration: evictUnderperformingValidators() call site moved out of
    ///        _applyReconfiguration into checkAndStartTransition / governanceReconfigure
    ///        (fixes DKG synchronization)
    ///      - ValidatorConfig: autoEvictThreshold(uint256) renamed to
    ///        __deprecated_autoEvictThreshold; new autoEvictThresholdPct(uint64) appended.
    ///        PendingConfig struct gets new field appended.
    ///      - GBridgeReceiver: _processedNonces mapping replaced with
    ///        __deprecated_processedNonces gap; isProcessed() and AlreadyProcessed removed.
    /// @param gbridgeReceiver Dynamic GBridgeReceiver instance address (deployed by Genesis)
    function epsilon(
        address gbridgeReceiver
    ) internal pure returns (HardforkDef memory def) {
        def.name = "epsilon";
        def.fromTag = "gravity-testnet-v1.3";

        def.upgrades = new ContractUpgrade[](4);
        def.upgrades[0] = ContractUpgrade(SystemAddresses.VALIDATOR_MANAGER, "ValidatorManagement");
        def.upgrades[1] = ContractUpgrade(SystemAddresses.RECONFIGURATION, "Reconfiguration");
        def.upgrades[2] = ContractUpgrade(SystemAddresses.VALIDATOR_CONFIG, "ValidatorConfig");
        def.upgrades[3] = ContractUpgrade(gbridgeReceiver, "GBridgeReceiver");

        // No PostActions:
        //   - GBridgeReceiver slot 0 keeps existing data as __deprecated_processedNonces gap
        //   - ValidatorConfig new autoEvictThresholdPct field is appended (zero-initialized);
        //     governance must call setForNextEpoch post-hardfork to set a meaningful value
    }
}
