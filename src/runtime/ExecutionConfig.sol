...
/// @author Gravity Team
/// @notice Configuration parameters for VM execution
...
///      The config is stored as opaque bytes (BCS-serialized), interpreted off-chain by VM.
contract ExecutionConfig {
    ...
    event ExecutionConfigUpdated(bytes32 indexed configHash);

    /// @notice Emitted when pending configuration is set by governance
    /// @param configHash Hash of the pending configuration
    event PendingExecutionConfigSet(bytes32 indexed configHash);

    /// @notice Emitted when pending configuration is cleared (applied or removed)
    event PendingExecutionConfigCleared();

    // ========================================================================
    // INITIALIZATION
    // ========================================================================

    /// @notice Initialize the execution configuration
    ...
    function initialize(
        ...
        if (_initialized) {
            revert Errors.ExecutionConfigAlreadyInitialized();
        }
        ...
        emit ExecutionConfigUpdated(keccak256(config));
    }
    ...
    function setForNextEpoch(
        ...
        emit PendingExecutionConfigSet(keccak256(newConfig));
    }
    ...
    function applyPendingConfig() external {
        ...
        emit ExecutionConfigUpdated(keccak256(_currentConfig));
        emit PendingExecutionConfigCleared();
    }
    ...
    function _requireInitialized() internal view {
        if (!_initialized) {
            revert Errors.ExecutionConfigNotInitialized();
        }
    }
}
