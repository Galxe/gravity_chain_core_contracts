...
contract PolymarketBinaryMarket is ReentrancyGuard {
    ...
    function refund(
        ...
        amount = _totalUserStake(marketId, msg.sender);
    ...
    function _validateCreateMarketParams(
        ...
        if (
            ref.sourceType != SOURCE_TYPE_POLYMARKET_SETTLEMENT || ref.mirrorId == 0 || ref.conditionId == bytes32(0)
                ...
                || ref.mode != SettlementMode.SingleConditionBinary || ref.outcomeSlotCount != BINARY_OUTCOME_COUNT
                || ref.slotToOutcome.length != BINARY_OUTCOME_COUNT
        ) {
            revert InvalidSettlementRef();
        }
    ...
    function _totalUserStake(
        ...
        return userStake[marketId][user][uint8(BinaryOutcome.No)] + userStake[marketId][user][uint8(BinaryOutcome.Yes)];
    }
