...
#
# NOTE on v1.5 branch: Delta-era bytecode is superseded by Epsilon + Zeta on
# StakingConfig / ValidatorManagement / Governance. SYSTEM_CONTRACTS is left
# empty here so Phase-1 bytecode hash comparison is skipped. The smoke tests
# below check Delta-introduced observables that are still present on v1.5.

HARDFORK_DISPLAY_NAME="Delta Hardfork"

# ── System contracts upgraded in this hardfork ────────────────────────
# Delta bytecodes no longer match current main source (Epsilon / Zeta
# regenerated these). Hash comparison is intentionally skipped.
SYSTEM_CONTRACTS=()
...
run_smoke_tests() {
    ...
    # getPendingConfig signature: (hasPending, (minStake, lockup, unbonding, __deprecated_minProposalStake))
    # The 4th tuple element is a deprecated storage gap kept for Gamma→Delta hardfork slot compatibility.
